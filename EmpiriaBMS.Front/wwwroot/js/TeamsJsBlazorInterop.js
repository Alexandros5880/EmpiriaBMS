...
// Not Interapt Helpper Generall Functions

// Load saved positions and sizes on page load
function loadSavedPositionsAndSizes(gridClass) {
    const items = document.querySelectorAll(gridClass);
    items.forEach(item => {
        const id = item.id;
        const savedData = JSON.parse(localStorage.getItem(id));
        if (savedData) {
            item.style.top = savedData.position.top + 'px';
            item.style.left = savedData.position.left + 'px';
            item.style.width = savedData.size.width + 'px';
            item.style.height = savedData.size.height + 'px';
        }
    });
}

// Saved positions and sizes on page load
function savePositionsAndSizes(gridClass) {
    // Select all items in the specified grid class
    const items = document.querySelectorAll(gridClass);

    items.forEach(item => {
        const id = item.id; // Get the ID of the current item
        ...
        // Save the data in local storage with the item's ID as the key
...
// Dragable and Resizable Compoments
export function initializeGridStack() {
    document.addEventListener('DOMContentLoaded', function () {
        ...
        grid.on('change', function (event, items) {
            ...
            DotNet.invokeMethodAsync('EmpiriaBMS.Front', 'SaveLayout', layout);
        });
    });
}
